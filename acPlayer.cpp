...
#define LEARNING_RUNS 1000
//#define USE_ICO

acPlayer::acPlayer() : gaussDistribution(0, 1.f), newGame(true), numRuns(0), logFile("AClog.txt")
{
    ...
    for(int i = 0; i < 4; ++i)
    {
        ...
        */
        ...
        epsilon[i] = gaussDistribution(generator);
    }
    ...
    offsets = { {0.f, 1.f}, {0.f, 1.f}, {-6.f, -2.f, 0.f, 2.f, 6.f}, {0.f, 1.f} }; //inputStart, inputSpecialStep, inputDangerChange, inputFinishFail
    devs = {0.1f, 0.1f, 0.5f, 0.1f};
...
int acPlayer::make_decision(){
    ...
    for(size_t i = 0; i < eligible.size(); ++i)
    {
        ...
        {
            int pos = posStart[j];
            bool safe = (myPos == -1 || myPos == 0 || myPos > 50 || inputSpecialStep[eligible[i]] > 0);
            if(safe)
            {
                // no danger
            }
        ...
        //lastInputVec[eligible[i]] = {inputStart[eligible[i]], inputSpecialStep[eligible[i]], inputDangerChange[eligible[i]], 0};
        //fann_type inputs[4] = {inputStart[eligible[i]], inputSpecialStep[eligible[i]], inputDangerChange[eligible[i]], 0};
        fann_type* output = fann_run(actor[eligible[i]], inputs);
#ifndef USE_ICO
        if(numRuns < LEARNING_RUNS)
        {
            output[0] += epsilon[eligible[i]];
        }
#endif
...
void acPlayer::runCritic()
{
    ...
    for(size_t l = 0; l < 4; ++l)
    {
        ...
        for(size_t i = 0; i < midNeurons.size(); ++i)
        {
            ...
            prevV[l] += v_j[i]*prevY_j[i];
        }

        float gaussNoise = gaussDistribution(generator);
        prevEpsilon[l] = epsilon[l];
        epsilon[l] = gaussNoise*std::min(1.f, std::max(0.f, (V_MAX - prevV[l])/(V_MAX - V_MIN)));

        // NEW INPUTS, ONLY IF THIS FIGURE WAS CHOSEN TO ACT LAST TURN ---------------------------------------------------------------
        if(l == lastDecision)
        {
            Eigen::VectorXf x(numInputs);
            for(size_t i = 0; i < numInputs; ++i)
            {
                x[i] = lastInputVec[lastDecision][i];
            }
            ...
            float delta = reward[lastDecision] + 0.5f*V - prevV[lastDecision];

            std::vector<float> dw;
            for(size_t i = 0; i < numInputs; ++i)
            {
                dw.push_back(LEARNING_RATE*prevEpsilon[lastDecision]*delta*prevX[i]);
            }
            ...
            for(size_t i = 0; i < midNeurons.size(); ++i)
            {
                ...
                if(midNeurons[i].weight + dv[i] < V_MAX && midNeurons[i].weight + dv[i] > V_MIN)
                {
...
void acPlayer::start_turn(positions_and_dice relative){
    ...
    for(size_t i = 0; i < 4; ++i)
    {
        ...
        if(inputDangerChange[i] > 0 && posStart[i] == -1 && prevPosStart[i] != -1 && lastDecision == i)
        {
            reward[i] += -1.f;
        }
        if(inputFinishFail[i] > 0 && lastDecision == i)
